...
There is just a single external dependency of AVX-Framework, besides .Net 8.0: YamlDotNet. Dotnet was chosen as it is open-source and it is cross-platform (I flirted with C++, but it is far too cumbersome, when compared to C#). YamlDotNet was chosen for its ability to parse/generate YAML, along with its liberal open source license. P/Invoke serve as the glue to facilitate efficient communication from C# to Rust. There are internal dependencies; those also are all open source and in-proc (maintained by AV Text Ministries; located at https://github.com/kwonus).  
